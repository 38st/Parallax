...
| Claude account tracking | Verified preview | Each tracked record uses a provider/account-specific Claude Code configuration directory and reports only parsed live `/usage` windows. Missing provider usage remains explicitly unavailable. |
...
| Localization | Verified | The census covers 953 source keys and 1,091 literals. English and Spanish each contain 953 keys; dynamic keys, unknown interpolations, new debt, and allowlisted debt are all zero. |
...
| Full Swift test suite with warnings as errors | PASS — 1,187 tests, 0 failures, 1 foreground-activation capability skip |
| Fresh isolated coverage suite | PASS — 1,187 tests, 0 failures, 1 capability skip |
| Product line coverage | PASS — 44,928 / 69,528 lines (64.6186%); floor 30,029 / 56,525 (53.1252%) |
| Localization checker | PASS — 953/953 keys in English and Spanish, zero debt |
| Localization checker contracts | PASS — 13/13 |
| CI evidence hygiene | PASS — 9/9 |
