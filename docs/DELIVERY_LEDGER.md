...
| Claude account tracking | Verified preview | Claude Code uses one macOS-user-shared credential. Parallax permits one active Claude record, keeps record-specific configuration/session state, serializes Claude operations provider-wide, and reports only parsed live `/usage` windows. Legacy rows remain stored but are not treated as independent sign-ins. |
...
| Full Swift test suite with warnings as errors | PASS — 1,204 tests, 0 failures, 1 foreground-activation capability skip |
| Fresh isolated coverage suite | PASS — 1,204 tests, 0 failures, 1 capability skip |
| Product line coverage | PASS — 45,167 / 69,654 lines (64.8448%); floor 30,029 / 56,525 (53.1252%) |
