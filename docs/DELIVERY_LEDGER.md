...
| CI-005 | Add separate ASan and TSan lanes. | CI-001 | In progress | Separate local ASan and TSan runs each pass 712/712 with clean diagnostics and isolated scratch cleanup; run 31326376504 is green at `5adf8d2` on PR #10. Merge remains pending protected review. |
...
PROV-002 implementation is active on branch
`codex/wave-3-process-identity`. Its dependency baseline is `5adf8d2`, for
which [PR #10](https://github.com/38st/Parallax/pull/10) run
[31326376504](https://github.com/38st/Parallax/actions/runs/31326376504)
is green. That run predates PROV-002 and is not PROV-002 acceptance evidence.
...
or attributed to the requested space automatically.
...
| PROV-002 | `/root/macos_architecture_audit`; `Sources/Parallax/Services/LaunchConfigurationCompiler.swift`, `Sources/Parallax/Services/LaunchProcessProvenance.swift`, `Sources/Parallax/Services/ApplicationLauncher.swift`, `Sources/Parallax/Stores/ProfileActivityRegistry.swift`, optional new `Sources/Parallax/Services/WorkspaceApplicationLaunchAuthority.swift`; `Tests/ParallaxTests/LaunchConfigurationCompilerTests.swift`, `Tests/ParallaxTests/LaunchProcessProvenanceTests.swift`, `Tests/ParallaxTests/WorkspaceProcessSnapshotterTests.swift`, `Tests/ParallaxTests/LaunchLifecycleTests.swift`, `Tests/ParallaxTests/ProfileActivityRegistryTests.swift`, `Tests/ParallaxTests/LaunchPreparationIntegrationTests.swift`, `Tests/ParallaxTests/LibraryStoreRelocationTests.swift`, optional new `Tests/ParallaxTests/WorkspaceApplicationLauncherProvenanceTests.swift` | False launch ownership or leaked safety receipt; revert PROV-002 launcher binding/claim changes as one atomic slice and restore the prior fail-closed preparation boundary—do not retain partial running-publication changes | Required/pending: `swift test --scratch-path /tmp/parallax-prov002 --filter 'LaunchProcessProvenanceTests\|WorkspaceProcessSnapshotterTests\|WorkspaceApplicationLauncherProvenanceTests\|LaunchConfigurationCompilerTests\|ProfileActivityRegistryTests' -Xswiftc -warnings-as-errors`; `git diff --check`. Matrix: pre-open snapshot failure opens nothing and acquires no lease; exact new; exact pre-existing; simultaneous singleton claims admit exactly one; returned-PID mismatch; same PID/start-second but different microsecond; missing/mismatched path or identifier; bundle identity changing between inspections; immediate exit; exact registry record cannot rebind PID; untracked prepared API cannot open. No result yet. | — | — | — | Active on `codex/wave-3-process-identity`; PR #10 run 31326376504 is green at dependency baseline `5adf8d2` but predates this implementation | Post-open indeterminacy may leave an app running; retain conservative safety evidence and do not publish success until exact disposition is proven |
