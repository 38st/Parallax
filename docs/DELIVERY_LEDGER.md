...
| SINGLE-001 | Refuse a pre-existing singleton by default rather than claiming a newly isolated space; present a truthful recovery message. | PROV-002 | In progress |
| ADAPTER-001 | Verify created, self-exiting, singleton, activation, and quit behavior with harmless disposable application fixtures. | PROV-004 | In progress |
...
PROV-004 implementation by `/root/security_integrity_audit` is committed and
pushed on the same branch at `18c14e430965490b446f91779190b93f14f03844`
in draft [PR #11](https://github.com/38st/Parallax/pull/11). Exact remote
[run 31331101008](https://github.com/38st/Parallax/actions/runs/31331101008)
is queued, so formal status remains **In progress**. Only an exact active
running launch with matching PID, start seconds and microseconds,
...
SINGLE-001 is assigned to `/root/ux_accessibility_audit`: an exact pre-existing
singleton is refused without publishing running state, history, last-opened
state, or any argument/environment delivery claim, and only that exact
pre-existing classification releases its launch lease. Indeterminate results
remain gated. Launch Services admission uses `activates = false`; any later
activation requires exact post-admission identity and an immediate recheck
before the OS call. Reachable recovery copy states these limits truthfully in
English and Spanish. The implementation and local review gates are complete,
and commit `c089504e1151a0f86ed5f9603f3a3a28f673bf31` is pushed on the
same branch in draft [PR #11](https://github.com/38st/Parallax/pull/11).
Exact [run 31332886344](https://github.com/38st/Parallax/actions/runs/31332886344)
is queued, so formal status remains **In progress**.

ADAPTER-001 is assigned to `/root/quality_release_audit`. Its initial exact
scope is two new files, `ProductionLaunchApplicationFixture.swift` and
`NSWorkspaceApplicationLauncherIntegrationTests.swift`. The local implementation
and review are complete for the always-provable matrix. The warnings-fatal suite
executed nine tests twice, with eight passes and one explicit activation
non-proof skip; the independent rerun produced the same 8-pass/1-skip result in
11.240 seconds and found no P0/P1 blocker. Activation remains unproven on the
current noninteractive host: requiring that capability fails red rather than
turning the skip into evidence. Formal status remains **In progress** pending a
capable interactive-host activation proof and protected merge. The additive
slice is committed at `f3d08fd57392496c31ca823c4e2585ed98699f0d`, pushed on
`codex/wave-3-process-identity`, and exact remote
[run 31334184282](https://github.com/38st/Parallax/actions/runs/31334184282)
is fully green for tests, ASan, TSan, coverage, production Keychain,
packaging, and artifact inspection; the credentialed signed-manual job was
truthfully skipped.
The first root integrated full run had one transient, order-dependent failure;
that failure remains part of the evidence. Two immediate consecutive full
`--skip-build` reruns then executed 799 tests with the one explicit activation
non-proof skip and zero failures, in 26.947 and 27.169 seconds. There is no
current unexpected failure, but the clean repeats do not erase the first result.
The later fresh-build failure is also retained: two tracked same-application
`NSWorkspace` completions under load could lose or delay the second callback and
leave that second request launching. The harness now prelaunches one real,
untracked same-application sibling and submits exactly one production-tracked
target, preserving the sibling/ownership assertion without depending on two
concurrent Launch Services completions. Two fresh adapter suites are green, and
root `swift test --scratch-path /tmp/parallax-wave4-set003-full3 --jobs 4 -Xswiftc -warnings-as-errors`
executed 943 tests with one truthful activation-capability skip and zero
failures. Independent read-only rereview approved the harness with no P0/P1/P2
finding.
...
| ID | Outcome | Dependencies | Status |
| --- | --- | --- | --- |
| SET-001 | Port the bounded versioned settings document, codec, and strict JSON preflight. | CI-001 | In progress |
| SET-002 | Port restrictive inspection, mutation locking, and locked primary inspection. | SET-001 | In progress |
| SET-003 | Port revision/digest CAS publication and restart-safe residual inventory. | SET-002 | In progress |
...
Wave 4 is active on branch `codex/wave-4-versioned-settings`. SET-001 is
assigned to `/root/macos_architecture_audit` as a five-file additive document,
strict-preflight, codec, and test slice. The codec must preserve accepted scalar
values exactly, must not invoke `DisplayNameValidator`, and must not create or
mutate runtime settings, `UserDefaults`, UI state, or persisted bytes. The local
implementation, focused tests, sanitizer checks, full suite, and independent
review are complete. Commit `d59dad78de443eb25bc01f18c061dda3c6033e9f`
is pushed on the same branch in draft
[PR #12](https://github.com/38st/Parallax/pull/12), with exact
[run 31334929942](https://github.com/38st/Parallax/actions/runs/31334929942)
fully green. Formal status remains **In progress** pending protected merge.
Schema v1 freezes its
appearance, visual-color, and visual-symbol raw-value allowlists. Unknown v1
values are invalid and retain exact recovery bytes. Future product enum
expansion must advance the schema rather than mutate v1 acceptance; historical
template-name scalars are the sole explicitly compatibility-preserved exception
and remain structurally bounded.

SET-002 is assigned to `/root/macos_architecture_audit` as an exact six-file
additive restrictive-inspection, mutation-lock, repository, and test slice.
Reads are descriptor-relative with `O_NOFOLLOW_ANY | O_CLOEXEC`, fixed to
`settings.json`, and bounded. Inspection never creates, repairs, or deletes.
The effective owner, exact `0700` Settings directory, exact `0600` single-link
regular lock and primary, no special bits or ACL, and exact descriptor/path/
ancestor identity are required through preflight, open, read, and postflight.
Mutation acquisition may stage/create only the private Settings directory and
lock, uses bounded nonblocking `flock`, and exposes an opaque same-thread,
expiring, nonreentrant authority whose sole operation is locked primary
inspection. Primary and locked reads permit at most 64 consecutive `EINTR`
results by default and reset the counter on read progress; status calls fail with
the typed system `EINTR` on attempt 65; and `flock` permits at most 256
consecutive `EINTR`/`EWOULDBLOCK`/`EAGAIN` attempts independent of timeout or a
frozen clock before returning the typed timeout. Acquisition failure performs no
name-based deletion: all `unlinkat`/cleanup-name paths are removed, and any
unclaimed staging, Settings, or lock artifact is deliberately preserved because
Darwin cannot condition deletion on the already-validated descriptor identity.
Normal unlock and descriptor close remain. Two final-boundary swap tests prove
both the moved original and replacement survive. Unknown, future, and invalid
bytes remain classified and preserved through SET-001, never published or
migrated. Local focused, sanitizer, full-suite, and independent review gates are
complete. Commit `bba47d5d2fc95e9313c9f39c4337d8b80da8eac1` is pushed
on the same branch in draft PR #12; exact
[run 31335878334](https://github.com/38st/Parallax/actions/runs/31335878334)
is in progress, so formal status remains **In progress** pending exact remote CI
and protected merge.

SET-003 is assigned to `/root/macos_architecture_audit` for revision-and-digest
CAS publication plus a restart-safe residual inventory. Publication accepts
only a missing primary or the exact current revision/SHA-256 token captured
under the same lock, requires canonical current-v1 target bytes and a valid next
revision, synchronizes and revalidates its restrictive temporary before a
single atomic no-replace/swap publication, synchronizes the Settings directory,
and postflights the exact target. Stale, future, invalid, or unavailable prior
evidence must reject before publication. Any post-effect failure is classified
as target, prior, neither, or indeterminate while retaining primary cause,
classification-read failure, close/lock-cleanup failures, and any displaced or
possible residual; ambiguity never authorizes deletion. The restart inventory
is a pinned, bounded, read-only scan of raw reserved publication names and exact
bytes, classifying current/future/corrupt/unavailable entries and returning
partial evidence for limits, races, unsafe metadata, system calls, or close
failures. It never repairs, normalizes, or deletes. This slice exposes repository
publication authority but does not connect `AppSettings`, `UserDefaults`, UI,
or migration. The implementation uses six of the approved maximum eight files:
two existing stores are modified and four publication/residual source and test
files are new; the two preauthorized existing test files required no edit. After
the FIFO remediation, opening the displaced prior after `RENAME_SWAP` uses
`O_NONBLOCK` while retaining full validation, so a swapped FIFO cannot block the
writer. The deterministic after-rename FIFO test completes with typed fail-closed
recovery and preserves the target, moved prior, and FIFO. Focused,
warnings-fatal build, sanitizer, fresh full-suite, and independent security
gates are complete; formal status remains **In progress** pending commit, push,
and exact remote CI.
...
| PROV-004 | `/root/security_integrity_audit`; exact task scope: `Sources/Parallax/Services/ApplicationInstanceController.swift`, `Sources/Parallax/Stores/LibraryStore+Activity.swift`, `Sources/Parallax/Stores/LibraryStore+LaunchLifecycle.swift`, `Sources/Parallax/Views/CompactApplicationHeaderView.swift`, `Sources/Parallax/Views/ParallaxMenuBarView.swift`, `Tests/ParallaxTests/ApplicationInstanceControllerTests.swift`, new `Tests/ParallaxTests/LibraryStoreProcessAuthorityTests.swift`, new `Tests/ParallaxTests/ProcessAuthorityPresentationTests.swift`, nine genuine keys in `Sources/Parallax/Resources/en.lproj/Localizable.strings` and `Sources/Parallax/Resources/es.lproj/Localizable.strings`; unchanged `Tests/ParallaxTests/LaunchLifecycleTests.swift` included in focused verification | Wrong-process activation, quit, association, lookup, or UI action; revert only this PROV-004 slice—do not revert or reopen committed PROV-002/003 launcher, provenance, or supervisor work | Local implementation complete / locally verified. `swift test --scratch-path /tmp/parallax-prov004 --filter 'ApplicationInstanceControllerTests\|LibraryStoreProcessAuthorityTests\|LaunchLifecycleTests\|ProcessAuthorityPresentationTests' -Xswiftc -warnings-as-errors`: 39/39; warnings-as-errors build passed; localization contracts 13/13 and checker reported zero new/stale issues; root `swift test --scratch-path /tmp/parallax-wave3-prov004-full --jobs 4 -Xswiftc -warnings-as-errors`: 779/779; `git diff --check`: clean. Matrix covers path/identifier discovery, missing/conflicting identity, stale labels, PID/start-microsecond reuse, path/identifier swaps before quit and across activation boundaries, synchronous quit, rejection rollback, exact lookup among PID-shaped fakes, wrong request/identity rejection, authority-revision presentation invalidation, and disabled external controls. | `18c14e430965490b446f91779190b93f14f03844` | `/root/macos_architecture_audit`; APPROVE with no P0/P1 blocker | Pushed on `codex/wave-3-process-identity` in draft [PR #11](https://github.com/38st/Parallax/pull/11); exact [run 31331101008](https://github.com/38st/Parallax/actions/runs/31331101008) queued | Only an exact active running launch is actionable; recovered/terminating or ambiguous same-PID state fails closed. An irreducible AppKit TOCTOU interval remains between final exact validation and the external activation/termination operation. Formal status remains In progress while the exact remote run is pending. |
| SINGLE-001 | `/root/ux_accessibility_audit`; exact scope: `Sources/Parallax/Services/ApplicationLauncher.swift`, `Sources/Parallax/Stores/LibraryStore+LaunchLifecycle.swift`, `Sources/Parallax/Views/ProfileListView.swift`, `Sources/Parallax/Views/ProfileEditorView+Components.swift`, `Sources/Parallax/Resources/en.lproj/Localizable.strings`, `Sources/Parallax/Resources/es.lproj/Localizable.strings`, `Tests/ParallaxTests/WorkspaceApplicationLauncherProvenanceTests.swift`, `Tests/ParallaxTests/LaunchPreparationIntegrationTests.swift`, `Tests/ParallaxTests/LaunchLifecycleTests.swift`, `Tests/ParallaxTests/ProfileActivityRegistryTests.swift`, `Tests/ParallaxTests/LibraryStoreRelocationTests.swift`, new `Tests/ParallaxTests/SingletonLaunchPolicyTests.swift` | False isolation or delivery claims, unsafe lease release, or wrong-process activation; revert only SINGLE-001 while preserving PROV-002/003/004 exact-identity gates | Local implementation complete / locally verified. Owner full suite: 790/790. Root `swift test --scratch-path /tmp/parallax-wave3-singleton-final --jobs 4 -Xswiftc -warnings-as-errors`: 790/790. Independent `swift test --scratch-path /tmp/parallax-single001-rereview --filter 'SingletonLaunchPolicyTests\|WorkspaceApplicationLauncherProvenanceTests\|LaunchPreparationIntegrationTests\|LaunchLifecycleTests' -Xswiftc -warnings-as-errors`: 52/52. Localization checker: 815 keys, EN 75, ES 75, zero new issues; `plutil` and `git diff --check` clean. Matrix covers exact pre-existing refusal, no running/history/last-opened or delivery claim, lease release only for exact pre-existing classification, indeterminate gating, truthful EN/ES recovery copy, `activates = false`, and exact post-admission identity plus immediate pre-activation recheck. | `c089504e1151a0f86ed5f9603f3a3a28f673bf31` | `/root/security_integrity_audit`; independent warnings-fatal 52/52; APPROVE with no P0/P1 blocker | Pushed on `codex/wave-3-process-identity` in draft [PR #11](https://github.com/38st/Parallax/pull/11); exact [run 31332886344](https://github.com/38st/Parallax/actions/runs/31332886344) queued | Formal status remains In progress while the exact remote run is pending. A final OS activation TOCTOU interval remains only after the immediate exact-identity recheck. Degraded bring-forward remains outside the accepted default policy. |
| ADAPTER-001 | `/root/quality_release_audit`; exact two-new-file scope: `ProductionLaunchApplicationFixture.swift`, `NSWorkspaceApplicationLauncherIntegrationTests.swift` | A test contacts or terminates a non-fixture app, leaks a process, or leaves artifacts; delete the two additive fixture/test files and preserve PROV-002/003/004/SINGLE-001 production behavior | Local implementation complete for the always-provable matrix. Owner ran the warnings-fatal focused suite twice: 9 executed, 8 passed, 1 explicit activation non-proof skip each time. Independent focused rerun: 9 executed, 8 passed, 1 explicit skip in 11.240s. Always-green proofs cover created process arguments/environment/full identity, self-exit lease release, singleton refusal with no second marker, cooperative quit preserving the sibling, deterministic compiler failure/timeout/callback-loss/unregistered cleanup, exact process/root cleanup, and no leaks; `git diff --check` clean. Root integrated full execution first had one transient, order-dependent failure, followed by two immediate `swift test --scratch-path /tmp/parallax-wave3-singleton-final --skip-build` reruns with 799 executed, 1 explicit activation non-proof skip, and 0 failures in 26.947s and 27.169s. A later fresh-build failure showed that two tracked same-app `NSWorkspace` completions under load could lose/delay the second callback and strand that request launching. The remediated harness prelaunches one real untracked same-app sibling and submits exactly one production-tracked target; two fresh adapter suites passed. Root `swift test --scratch-path /tmp/parallax-wave4-set003-full3 --jobs 4 -Xswiftc -warnings-as-errors`: 943 executed, 1 truthful activation-capability skip, 0 failures. Required activation command `PARALLAX_REQUIRE_REAL_ACTIVATION_TESTS=1 swift test --scratch-path /private/tmp/parallax-adapter001-review --filter NSWorkspaceApplicationLauncherIntegrationTests/testWorkspaceControllerActivatesOnlyTheExactTrackedInstance -Xswiftc -warnings-as-errors`: expected red, 1/1 failed with `requiredCapabilityUnavailable`, `currentProcessIsActive=false`, `activationPolicy=2`; explicitly not activation proof. | Base fixture commit `f3d08fd57392496c31ca823c4e2585ed98699f0d`; harness remediation pending commit | `/root/macos_architecture_audit`; final independent read-only APPROVE with no P0/P1/P2 after one-tracked-target remediation | Base commit pushed on `codex/wave-3-process-identity`; [run 31334184282](https://github.com/38st/Parallax/actions/runs/31334184282) fully green for tests, ASan, TSan, coverage, production Keychain, package, and artifact inspection; signed manual skipped truthfully. Remediation remote CI pending. | Both earlier failures remain retained evidence; no current unexpected failure remains after the one-tracked-target redesign and fresh 943-test run. Real activation proof still requires a capable interactive host; the explicit skip is not acceptance evidence. |
| SET-001 | `/root/macos_architecture_audit`; exact five-new-file scope: `Sources/Parallax/Models/SettingsDocument.swift`, `Sources/Parallax/Stores/StrictJSONPreflight.swift`, `Sources/Parallax/Stores/SettingsDocumentCodec.swift`, `Tests/ParallaxTests/StrictJSONPreflightTests.swift`, `Tests/ParallaxTests/SettingsDocumentCodecTests.swift` | Decode incompatibility or unintended persistence/runtime mutation; delete all five additive files—SET-001 creates no settings file and touches no `UserDefaults`, runtime state, UI, or persisted bytes | Local implementation complete / locally verified. `swift test --scratch-path /private/tmp/parallax-set001 --filter 'StrictJSONPreflightTests\|SettingsDocumentCodecTests' -Xswiftc -warnings-as-errors`: 39/39, zero failures in 3.857s (24 codec, 15 preflight); maximum 4,096-template/4,096-visual document 0.823s and 4 MiB preflight average 0.099s. Fresh `swift build --scratch-path /private/tmp/parallax-set001-build -Xswiftc -warnings-as-errors`: passed in 13.64s. Focused ASan: 39/39 in 7.148s; focused TSan: 39/39 in 9.713s. Root `swift test --scratch-path /tmp/parallax-wave4-set001-full --jobs 4 -Xswiftc -warnings-as-errors`: 838 executed, one explicit prior activation-capability skip, zero failures. Forbidden-authority/source-caller, whitespace, and diff checks clean. Independent `swift test --scratch-path /tmp/parallax-set001-review --filter 'SettingsDocumentCodecTests\|StrictJSONPreflightTests' -Xswiftc -warnings-as-errors`: 39/39; 4 MiB preflight 0.093s and maximum document 0.593s. Matrix covers size/depth/type/version/duplicate/strict JSON, current/future/invalid exact bytes, deterministic round-trip/canonical encoding, hostile bounds, scalar preservation, source decoupling, and performance. | `d59dad78de443eb25bc01f18c061dda3c6033e9f` | `/root/security_integrity_audit`; APPROVE with no P0/P1/P2 blocker; independent warnings-fatal 39/39 | Pushed on `codex/wave-4-versioned-settings` in draft [PR #12](https://github.com/38st/Parallax/pull/12); exact [run 31334929942](https://github.com/38st/Parallax/actions/runs/31334929942) fully green | Formal status remains In progress pending protected merge. Schema v1 intentionally fixes appearance, visual-color, and visual-symbol raw-value allowlists; unknown v1 values are invalid and retain exact recovery bytes. Future product enum expansion must advance the schema rather than mutate v1 acceptance. Historical template-name scalars are the sole compatibility-preserved exception and remain structurally bounded. This additive slice is not runtime integration or migration. |
| SET-002 | `/root/macos_architecture_audit`; exact six-new-file scope: `Sources/Parallax/Stores/SettingsPrimaryFileAccess.swift`, `Sources/Parallax/Stores/SettingsPrimaryMutationLock.swift`, `Sources/Parallax/Stores/SettingsRepository.swift`, `Tests/ParallaxTests/SettingsRepositoryTests.swift`, `Tests/ParallaxTests/SettingsPrimaryMutationLockTests.swift`, `Tests/ParallaxTests/SettingsPrimaryLockedInspectionTests.swift` | Lockout, unsafe read, leaked authority, or persistence side effect; delete all six additive files and retain SET-001 codec evidence | Local implementation complete / locally verified. `swift test --scratch-path /private/tmp/parallax-set002 --filter 'SettingsRepositoryTests\|SettingsPrimaryFileAccessTests\|SettingsPrimaryMutationLockTests\|SettingsPrimaryLockedInspectionTests' -Xswiftc -warnings-as-errors`: 59/59, zero failures; warnings-fatal build passed; focused ASan 59/59; focused TSan 59/59. Root `swift test --scratch-path /tmp/parallax-wave4-set002-full --jobs 4 -Xswiftc -warnings-as-errors`: 897 executed, one prior activation-capability skip, zero failures. Matrix covers descriptor-relative `O_NOFOLLOW_ANY \| O_CLOEXEC` bounded reads; owner/exact `0700`/`0600`/type/link/ACL/special-bit checks; descriptor/path/ancestor identity and swaps; growth; bounded contention; cleanup and system-call failures; same-thread expiring nonreentrant authority; descriptor reuse; read/status/flock no-progress ceilings. All name-based acquisition-failure deletion and `unlinkat` cleanup were removed; two deterministic post-final-validation swaps prove both moved original and replacement survive. Independent warnings-fatal rereview of the same four XCTest classes: 59/59. | `bba47d5d2fc95e9313c9f39c4337d8b80da8eac1` | `/root/security_integrity_audit`; APPROVE with no P0/P1 after cleanup P1 remediation; independent 59/59 | Pushed on `codex/wave-4-versioned-settings` in draft [PR #12](https://github.com/38st/Parallax/pull/12); exact [run 31335878334](https://github.com/38st/Parallax/actions/runs/31335878334) in progress | Formal status remains In progress while exact remote CI is pending. Inspection never creates/repairs/deletes; mutation acquisition may leave unclaimed private staging/Settings/lock artifacts on failure. Their recovery requires a separate explicitly authorized design that can preserve identity safety; SET-002 never publishes or migrates settings. |
| SET-003 | `/root/macos_architecture_audit`; actual six-file scope: modified `Sources/Parallax/Stores/SettingsPrimaryMutationLock.swift`, `Sources/Parallax/Stores/SettingsRepository.swift`; new `Sources/Parallax/Stores/SettingsPrimaryPublication.swift`, `Sources/Parallax/Stores/SettingsPublicationResidualInventory.swift`, `Tests/ParallaxTests/SettingsRepositoryMutationTests.swift`, `Tests/ParallaxTests/SettingsPublicationResidualInventoryTests.swift`. Preauthorized `Tests/ParallaxTests/SettingsPrimaryMutationLockTests.swift` and `Tests/ParallaxTests/SettingsRepositoryTests.swift` required no edit. | Lost update, misclassified publication, or erased recovery evidence; revert SET-003 publication/inventory changes as one slice to restore SET-002 inspection-only behavior, preserving every residual artifact | Local implementation complete / locally verified. `swift test --scratch-path /private/tmp/parallax-set003 --filter 'SettingsRepositoryTests\|SettingsPrimaryFileAccessTests\|SettingsPrimaryMutationLockTests\|SettingsPrimaryLockedInspectionTests\|SettingsRepositoryMutationTests\|SettingsPublicationResidualInventoryTests' -Xswiftc -warnings-as-errors`: 105/105, zero failures (59 SET-002 plus 28 publication/CAS and 18 residual tests); warnings-fatal build passed; focused ASan 105/105; focused TSan 105/105; diff/new-file whitespace and source/no-delete/no-runtime audits clean. Root `swift test --scratch-path /tmp/parallax-wave4-set003-full3 --jobs 4 -Xswiftc -warnings-as-errors`: 943 executed, one truthful prior activation-capability skip, zero failures. Matrix covers exact revision/SHA CAS, stale/future/corrupt/unavailable/overflow rejection, canonical next revision, restrictive temporary/write/sync/proof, single-attempt rename, exact effect classification with retained causes, concurrent writers, residual preservation, and pinned bounded raw-byte restart inventory. `O_NONBLOCK` displaced-prior validation remediates the after-swap FIFO hang while preserving target, moved prior, and FIFO. Independent security rereview of the final selector: 105/105. | — — implementation commit pending | `/root/security_integrity_audit`; APPROVE with no P0/P1 after `O_NONBLOCK` FIFO P1 remediation; independent 105/105 | — — push and exact remote CI pending | Formal status remains In progress pending commit, push, and exact remote CI. Residual equality is observation only, never provenance; genuine/planted byte-identical, future, corrupt, unsafe, changed, or partial residuals remain evidence and are never auto-deleted. Rename is single-attempt because `EINTR` is post-effect ambiguous. No runtime/UI/migration integration belongs to SET-003. |
