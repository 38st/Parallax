...
review are complete; formal status remains **In progress** pending commit, push,
and exact remote CI. Schema v1 freezes its appearance, color, and symbol raw-value
allowlists. A future accepted value requires a new schema-version decision and
decoder path rather than silently widening what v1 means; until then, v1
truthfully rejects that value.
